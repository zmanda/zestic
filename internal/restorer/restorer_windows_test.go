...
func setup(t *testing.T, nodeInfo NodeTestInfo, testDir string, createExisitingFile bool, existingNodeInfo NodeTestInfo) *Restorer {
	...
	getFileAttributes := func(attr *FileAttributes, isDir bool) (genericAttributes []restic.Attribute) {
		if attr == nil {
			return
		}
	...
	getAdsAttributes := func(path string, hasAds, isAds bool) restic.Attribute {
		if isAds {
			return restic.NewGenericAttribute(restic.TypeIsADS, []byte(fs.TrimAds(path)))
		} else if hasAds {
			//Find ads names by recuresevely searching through nodes
...
func createExisting(t *testing.T, testDir string, nodeInfo NodeTestInfo) {
	...
	if !nodeInfo.IsDirectory {
		...
		createFile(t, nodeInfo.attributes.Encrypted, filepath, nodeInfo.DataStreamInfo)
	} else {
	...
	if len(nodeInfo.AdsStreams) > 0 {
		for _, stream := range nodeInfo.AdsStreams {
			...
			createFile(t, nodeInfo.attributes.Encrypted, filepath, stream)
		}
	}
...
func createFile(t *testing.T, isEncrypted bool, filepath string, stream DataStreamInfo) {
...
func findAdsNamesRecursively(nodesMap map[string]Node, path string, adsNames []string) []string {
	for name, node := range nodesMap {
		if fs.TrimAds(name) == path && name != path {
